...
export class ImageCard {
  ...
  imageSelected(dialogType: string, evt: PointerEvent) {
    ...
    let detail
    if (dialogType === 'map') {
      let location = this._manifest.metadata.find(md => Object.values(md.label).flat().indexOf('location') >= 0)
      detail = {type: dialogType, coords: location ? Object.values(location.value).flat()[0] : null}
    } else {
      detail = {type: dialogType, manifest: this._manifest}
    }
    this.imageSelectedEvent.emit(detail)
  }
