...
export class MapViewer {
  @Prop() overlay: string
  @Prop({ mutable: true, reflect: true }) zoom: number = 7
  ...
  getLayerStrings() {
    let layerObjs = Array.from(this.el.querySelectorAll('li')).map(item => this._toObj(item.firstChild.textContent))
    if (this.marker) layerObjs.push(this._toObj(this.marker))
    this._layerObjs = layerObjs
  }
