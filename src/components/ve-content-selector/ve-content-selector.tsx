...
export class ContentSelector {
  ...
  clicakbleChildren() {
    return <div class="dirs">
      {this.dirList.filter(item => this.mode === 'essays' || item.type === 'dir').map(item => 
    ...
    return [
      this.ready
        ? <section class="content-path" style={{position: 'relative'}}>
            { this.workspace() }
            { this.dirList.filter(item => this.mode === 'essays' || item.type === 'dir').length > 0 && <sl-divider></sl-divider> }
            { this.dirList.length > 0 && this.clicakbleChildren() }
          </section>
        : <section class="content-path" style={{position: 'relative'}} innerHTML={this.notReadyText}></section>
        ,
